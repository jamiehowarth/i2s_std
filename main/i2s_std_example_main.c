...
static const uint8_t s_sample_audio[] = {
...
static void i2s_example_write_task(void *args)
{
    ...
    /* Assign w_buf */
    if (EXAMPLE_BUFF_SIZE != sizeof(s_sample_audio)) {
        ESP_LOGE(TAG, "The size of the sample audio is not equal to the buffer size");
        free(w_buf);
        vTaskDelete(NULL);
        return;
    }
